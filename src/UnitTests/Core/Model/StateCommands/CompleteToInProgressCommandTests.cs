using ClearMeasure.Bootcamp.Core.Model;
using ClearMeasure.Bootcamp.Core.Model.StateCommands;
using ClearMeasure.Bootcamp.Core.Services;
using Shouldly;

namespace ClearMeasure.Bootcamp.UnitTests.Core.Model.StateCommands;

[TestFixture]
public class CompleteToInProgressCommandTests : StateCommandBaseTests
{
    [Test]
    public void IsValid_WhenStatusIsNotComplete_ReturnsFalse()
    {
        var order = new WorkOrder();
        order.Status = WorkOrderStatus.InProgress;
        var employee = new Employee();
        order.Assignee = employee;

        var command = new CompleteToInProgressCommand(order, employee);
        command.IsValid().ShouldBeFalse();
    }

    [Test]
    public void IsValid_WhenStatusIsCompleteAndUserIsNotAssignee_ReturnsFalse()
    {
        var order = new WorkOrder();
        order.Status = WorkOrderStatus.Complete;
        var employee = new Employee();
        order.Assignee = employee;

        var command = new CompleteToInProgressCommand(order, new Employee());
        command.IsValid().ShouldBeFalse();
    }

    [Test]
    public void IsValid_WhenStatusIsCompleteAndUserIsAssignee_ReturnsTrue()
    {
        var order = new WorkOrder();
        order.Status = WorkOrderStatus.Complete;
        var employee = new Employee();
        order.Assignee = employee;

        var command = new CompleteToInProgressCommand(order, employee);
        command.IsValid().ShouldBeTrue();
    }

    [Test]
    public void Execute_SetsStatusToInProgress()
    {
        var order = new WorkOrder();
        order.Number = "123";
        order.Status = WorkOrderStatus.Complete;
        var employee = new Employee();
        order.Assignee = employee;

        var command = new CompleteToInProgressCommand(order, employee);
        command.Execute(new StateCommandContext());

        order.Status.ShouldBe(WorkOrderStatus.InProgress);
    }

    [Test]
    public void Execute_ClearsCompletedDate()
    {
        var order = new WorkOrder();
        order.Number = "123";
        order.Status = WorkOrderStatus.Complete;
        order.CompletedDate = DateTime.Now;
        var employee = new Employee();
        order.Assignee = employee;

        var command = new CompleteToInProgressCommand(order, employee);
        command.Execute(new StateCommandContext());

        order.CompletedDate.ShouldBeNull();
    }

    protected override StateCommandBase GetStateCommand(WorkOrder order, Employee employee)
    {
        return new CompleteToInProgressCommand(order, employee);
    }
}
